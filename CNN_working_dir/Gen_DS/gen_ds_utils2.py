...
def img_proc(base_path, ori_file_dir, dir_name, dir_name1, dir_name2, num_make_images, num_pixel, noise_factor, WB_option, WB_val, exp_img_option, overlap=0):
    ...
    file_name = [f"set{i+1}" for i in range(file_num)]
    ...
    for x in range(len(noise_factor_name)):
        ...
        for k in range(file_num):
            ...
            ori_X, ori_Y = ori_img.shape[1], ori_img.shape[0]
            if ori_X != ori_Y:
                print("Shape of the image is not square. Cropping to square.")
                ori_img = ori_img[:min(ori_X, ori_Y), :min(ori_X, ori_Y)]
            ...
            '''
            # Calculate stride and updated num_make_images
            stride = num_pixel - overlap
            image_size = resized_img.shape[0]  # Assuming square
            new_num_make_images = (image_size - overlap) // stride + 1

            print(f"Updated num_make_images: {new_num_make_images}, with stride: {stride}")

            # Generate sliced image names
            sliced_np_name = []
            for y in range(0, resized_img.shape[0] - num_pixel + 1, stride):
                for x in range(0, resized_img.shape[1] - num_pixel + 1, stride):
                    if len(noise_factor_name) > 1:
                        sliced_np_name.append(f"{noise_factor_name[x]}_{file_name[k]}_{y}_{x}.npy")
                    else:
                        sliced_np_name.append(f"{file_name[k]}_{y}_{x}.npy")

            # Slice and process images
            for idx, (y, x) in enumerate(
                [(y, x) for y in range(0, resized_img.shape[0] - num_pixel + 1, stride)
                 for x in range(0, resized_img.shape[1] - num_pixel + 1, stride)]
            ):
                # Image slicing
                re_img = resized_img[y:y + num_pixel, x:x + num_pixel].copy()

                # Normalization
                img_norm = re_img.astype(np.float32) / 255

                # White balance and noise application
                if WB_val[0] == 0:
                    # Pre-processing White Balance
                    White_Balance(img_norm, WB_option, WB_val)
                    img_noisy = Noise_effect(exp_img_option, img_norm, noise_factor, x)
                elif WB_val[0] == 1:
                    ...
                    # Default: Post-processing White Balance
                    img_noisy = Noise_effect(exp_img_option, img_norm, noise_factor, x)
                    White_Balance(img_noisy, WB_option, WB_val)

                # Save sliced image
                np_file_name = sliced_np_name[idx]
                np_file_path0 = os.path.join(dir_path, np_file_name)
                np.save(np_file_path0, re_img)
